    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await axios.post(`${SERVERURL}/api/v1/acceptBlog`, formdata, {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await axios.post(`${SERVERURL}/api/v1/rejectBlog`, formdata, {
            headers: headers, // Include the headers object in the request configuration
          });
          if (response.data.success === true) {
            // Update the state or perform any action on successful verification
...
filtered2 && (
  ...
  {filtered2.map((item, index) => (
    <tr key={index} className="border-b border-gray-200 transition-colors hover:bg-gray-100">
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">  <div className="pictureee">
    <img
