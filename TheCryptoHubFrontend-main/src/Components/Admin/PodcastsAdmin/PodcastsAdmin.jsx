...
import PodcastsTable from "../PodcastsListing/PodcastsTable";
  ...
  useEffect(() => {
    ...
    const fetchPodcastsData = async () => {
      try {
        ...
        const response = await axios.get(`${SERVERURL}/api/v1/getPodcasts`);
        ...
        <div className="vehicleAdminMain">
          ...
          {!loading && (
            <div className="topTableAdminVehicles">
              <PodcastsTable driverData={driversData} />
            </div>
          )}
        </div>
      </div>
